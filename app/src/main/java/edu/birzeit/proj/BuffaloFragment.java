...
public class BuffaloFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        RelativeLayout rootLayout = rootView.findViewById(R.id.buffaloRootLayout);
...
//
                                ...
                                if (ordColumnIndex != -1) {
                                    while (cursor.moveToNext()) {
                                        ...
                                        if (extra == null || extra.isEmpty()) {
                                            order += "\nBuffalo Chicken Pizza " + s + " no Extra\n"+ "ordered in:"+ currentDate +"\n\n";
                                        }
                                        else {
                                            order += "\nBuffalo Chicken Pizza " + s +"  "+ extra +"\n" +"ordered in:"+ currentDate+"\n\n";
                                        }
                                        dataBaseHelperOrder.settingOrder(emailUSER, order);
                                    }
        ...
        boolean isFavorite = restoreFavoriteStatusFromSharedPreferences("Buffalo");
        if (isFavorite) {
        ...
        heartButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                boolean isFavorite = toggleFavoriteStatus("Buffalo");
                // Update SharedPreferences with the new favorite status
                saveFavoriteStatusToSharedPreferences("Buffalo",isFavorite);
                // Start the transition animation
                if (isFavorite) {
                    transitionDrawable.startTransition(100);
                    addToFavorites("Buffalo");
                } else {
                    transitionDrawable.reverseTransition(100);
                    removeFromFavorites("Buffalo");
                }
            }
        });
